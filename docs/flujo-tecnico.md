...
- **Navegación unificada.** Cada sección lleva arriba un **calendario** (`site/_includes/calendario.njk` + `site/js/calendario.js`): abre en el día actual, resalta ese día y solo deja seleccionar los días con edición (el resto quedan deshabilitados); las fechas disponibles se inyectan como JSON y el JS renderiza el mes y navega entre meses dentro del rango publicado. Debajo, una barra secuencial (`nav-temporal.njk`) lleva al día anterior o siguiente. La fecha se muestra en un `dateline` sobre el contenido (no como título), y la cabecera (`base.njk`) resalta la pestaña activa entre Noticias, Repositorios y Archivo.
