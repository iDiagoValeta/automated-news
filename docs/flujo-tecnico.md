...
- **Navegación unificada.** Noticias y repositorios comparten la misma barra (`site/_includes/nav-temporal.njk`): día anterior, un selector con todas las fechas disponibles y día siguiente. La fecha se muestra en un `dateline` sobre el contenido (no como título), y la cabecera (`base.njk`) resalta la pestaña activa entre Noticias, Repositorios y Archivo.
